...
mod tests {
    ...
    async fn recovers_run_and_retry_to_fail_interrupted() {
        ...
        let t_run = task_repo::insert_task(&mut tx, b, 1, 1, "t").await.unwrap();
        let t_retry = task_repo::insert_task(&mut tx, b, 1, 1, "t").await.unwrap();
        let t_q = task_repo::insert_task(&mut tx, b, 1, 1, "t").await.unwrap();
