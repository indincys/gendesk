...
/// 每个比例的**配套精确尺寸**：只给了比例时用它补上 `size`，两个字段一起发。
///
/// 为什么必须补：线上实测（key `aixoras` · 模型 `gpt-image-2-1k`，批次 24–27 共 56 张）
/// 单发 `aspect_ratio: "9:16"` 回来的整批是 **1024×1024 正方形**——那个参数在这条链路上
/// 没起作用；补上 `size` 之后才是竖幅（941×1672）。v0.15.2 记的「画幅走 `aspect_ratio`
/// 而非 `size`」照的是端点文档，被实测推翻。两个都发且让二者自洽是代价最低的做法：
/// 多一个字段是零成本，少一个是一整批废图。
///
/// 取值同时满足**正好是该比例** + **两边都是 16 的倍数**。所以取不到 1080×1920 那类
/// 「手机分辨率」：1080÷16=67.5 会被端点整批拒，精确 9:16 的合法值是
/// …/1008×1792/**1152×2048**/1296×2304…。上游只保证比例、实际像素自己定
/// （9:16 实测回 941×1672），故这里给的是**比例的载体**而不是交付分辨率。
pub const RATIO_SIZES: &[(&str, &str)] = &[
    ("1:1", "1024x1024"),
    ("9:16", "1152x2048"),
    ("16:9", "2048x1152"),
    ("3:4", "1536x2048"),
...
/// 取某比例的配套精确尺寸。
pub fn companion_size(aspect_ratio: &str) -> Option<&'static str> {
    RATIO_SIZES
        .iter()
