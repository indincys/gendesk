 ...
 *
 * ## 这里为什么**仍然**没有「前面还有 N 人在排队」
 *
 * 位次现在是有的 —— 早前那条「即梦不回传」的结论是从 2026-07-27 那批从未入队的
 * 幽灵单上归纳出来的，已被推翻（健康的单从排队第一秒起就有 `queue_info`）。
 * 但它不属于这一条：这一条回答的是「**整条流水线**在不在动」，而位次是**每一单各自**
 * 的进度，两者在这块窄条里挤不到一起。逐单轨迹与全局排队速度各有去处
 * （`V2vQueueTrail` / `V2vQueueTrend`），排产要看的是那两块。
 *
 * ## 这一条用的是出片侧的信号
 *
